...
exports.resendOrderOtp = async (req, res, next) => {
    try {
        ...
        if (findAlreadySendOtp) {
            ...
            await createApplicationLog("Delivery", "resend delivery otp", {}, {}, loginUser?._id)
            await exports.sendOtp(deliveryAssignmentData?.customerData?.email, deliveryAssignmentData?.customerData?.firstName, deliveryAssignmentData?.customerData?.lastName, otp)
            return res.status(201).send({ status: 1, message: "Otp send successfully" })
        } else {
            return res.status(404).send({ status: 0, message: "Can not found previously sended otp." })
        }
