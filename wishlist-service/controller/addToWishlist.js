...
exports.addtoWishlist = async(req,res)=>{
    try {
        ...
        const productData = await Product.findOne({_id:productId,isDeleted:false,isPublic:true})
        ...
        if(!productData){
            return res.status(404).send({status:0,message:"Record not found",data:[]})
              ...
              if (wishlist.items.includes(productId)) {
                  return res.status(400).send({ status: 0, message: "Product already exists in the wishlist", data: [] });
              }
